...
During ongoing implementation and acceptance, user follow-ups belong to the current change by default. Preserve its identity and progress when refining requirements; revised briefs still follow the approval rules below. If a request pursues a separate outcome that does not fit, ask whether the user wants a new change before assigning or implementing that request. An explicit choice to create or switch changes needs no repeated confirmation. Determine the current change from the task context, not from unrelated work marked `in_progress` elsewhere in the project.
...
Use the human's explicit decision about the current brief to enter `approved`; an earlier decision still applies only while that brief is unchanged. An explicit request to implement that same brief can supply approval. A request to implement approves only what that request and its agreed references already determine. Material product decisions introduced by the agent require human approval before dependent implementation; routine technical choices within the agreed scope remain autonomous. Editing the brief is not itself approval. A generic earlier authorization does not approve subsequent revisions. Do not require a separate command or file write for every intermediate transition.
...
For unfinished work, retain only what helps continuation: what is done, what was verified, what remains, and the blocking question. At `ready_for_acceptance`, record a short outcome and evidence, including the checked revision or equivalent artifact identity when available. At `done`, retain a brief note of the human's acceptance of that result. Completion is evidence of accepted behavior at that point, not a perpetual guarantee about the latest code. On transition to `done`, automatically move `nanospec/changes/<name>.md` to `nanospec/changes/archive/<timestamp>_<name>.md`, using the actual UTC close-out time as `YYYY-MM-DDTHH-mm-ss.SSSZ`. Keep unfinished and canceled records outside this completion archive. The timestamp orders closure, not deployment or overlapping implementation. Preserve identity and evidence, repair navigation links affected by the move, and never overwrite an archive or add a second prefix. This is part of close-out, without a separate approval or invocation. Existing issues and briefs maintained elsewhere stay there; no duplicate artifact is required. Do not bulk-migrate history unless requested. Completed contents need no ongoing synchronization.
