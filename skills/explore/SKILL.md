...
Start with focused searches for the capability, its user-facing terms, entry points, and relevant tests. Search change titles and content recursively in both `nanospec/changes/` and its `archive/` for those terms and discovered identifiers; read the relevant records rather than the entire history. Archived filenames prefix the original name with UTC close-out time (`YYYY-MM-DDTHH-mm-ss.SSSZ_`). Search the original name too when following old references. The timestamp orders closure, not implementation or deployment, and archive location alone does not prove accepted behavior. Expand through dependencies that could materially affect the answer.
