...
For follow-up routing during Apply, request an additional filter while implementing a search page: the agent should revise the same change and its criteria, preserve progress, and apply the normal revision approval rules. Repeat during acceptance and after a handoff. Request an unrelated billing export: the agent should ask whether to create a new change before assigning or implementing that request; an explicit request for a separate change should not trigger the same question again. An unrelated `in_progress` record must not capture a new task. Concrete revisions routed through Shape should not restart the selection or review-mode menus, while a bare Shape invocation should still list candidates. These behavioral scenarios have not yet been run.
